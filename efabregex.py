...
DOMAIN = "efabregex"
if (
  "TEXTDOMAINDIR" in environ
  and gettext.find(DOMAIN, environ["TEXTDOMAINDIR"])
):
...
def _invalid_flags(s):
    return (
        any([x not in _valid_flag_chars for x in s])
        or s
        and s[-1] == "-"
        or "-0" in s
        or "--" in s
        or any([s.count(x) > 1 for x in _valid_flag_chars])
    )
...
def is_unterminated(s):
    ...
    if s[0] == "s" and s[1] in _valid_delimiters:
        # The previous test ensures it is not a valid substitution regex
        return True
...
def is_possibly_wrong_substitution(s):
    ...
    return (
        s
        and len(s) > 2
        and not is_substitution(s)
        and (
          is_substitution(s[1:])
          or is_pattern(s) and contains_invalid_flags(s)
        )
    )
...
def compile_substitution(s):
    ...
    repl = re.sub(
        r"\$(\d+)", lambda x: "\\" + x.group(1), repl
    )  # Replace \ for $ in backreferences
    try:
        regex_object = re.compile(pattern, _get_flags(flags))
