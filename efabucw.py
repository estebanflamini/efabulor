...
def get_valid_formats():
  try:
    output = subprocess.check_output(['unoconv', '--show'], text=True, stderr=subprocess.STDOUT)
    output = output.split('\n')
    ...
    print(_('Could not determine valid output formats. Reported error is: %s') % e, file=sys.stderr)
    sys.exit(1)
...
def main():
  ...
  if not args.fmt in get_valid_formats():
    sys.exit(_('%s is not a valid output format.') % args.fmt)

  cmd = ['unoconv', '-f', args.fmt, '--stdout', '--timeout', str(TIMEOUT), args.infile]

  for n in range(MAX_RETRIES):
    try:
      output = subprocess.run(cmd, check=True, capture_output=True).stdout
      ...
      if n == MAX_RETRIES - 1:
        ...
        print(_('Text conversion failed. Reported error is: %s') % e, file=sys.stderr)
        sys.exit(1)
