...
def get_args():
  ...
  parser.add_argument('-f', '-r', '--rules', '--transformation-rules', action='append',
                      metavar=_('<rule configuration file>'), required=True,
                      help=_('This option can appear multiple times. Files will be processed in the given order.'))
  parser.add_argument('--permissive', action='store_true', default=False)
  parser.add_argument('--log', nargs="?", const="-", default=None, metavar=_('<file>'),
                      help=_('Save log to this file (default is print to standard error)'))
  parser.add_argument('infile', nargs='?', default='-', metavar=_('<file to read input from>'))
  parser.add_argument('outfile', nargs='?', default='-', metavar=_('<file to send output to>'))
...
def process_args():
  ...
  if flags:
    try:
      ...
      sys.exit(_('Invalid global flags for regular expressions: %s') % flags)
  if '-' in args.rules:
...
def confirm_overwrite(file_name):
  if os.path.exists(file_name):
    if not os.path.isfile(file_name):
      ...
      if not confirm_action(_('File %s already exists. Overwrite it?') % file_name):
...
def set_encoding():
  ...
  input_encoding = 'utf-8-sig' if output_encoding.lower() in ['utf-8', 'utf8'] else output_encoding
...
def set_rules():
  ...
  for filename in args.rules:
    try:
      ...
      sys.exit(_('An error has occurred while reading file: %s. Reported error is: %s') % (filename, e))
    ...
    for x in content:
      ...
      if x and len(x) > 1 and (x[0] == '[' or x[-1] == ']'):
        if x[0] != '[' or x[-1] != ']':
          sys.exit(_('Error in file: %s. Unterminated header: %s') % (filename, x))
        # Can only be [...]
        if canonical_header(x[1:-1]) not in ['do', 'donot']:
          sys.exit(_('Error in file: %s. Wrong header: %s') % (filename, x))

    content = '\n'.join(content).strip()

    content = re.split(r'^(\[.+?\])$', content, flags=re.MULTILINE) # Split by section headers
    ...
    for block in content:
      if not block: # In case the splitting above returns any empty block
        pass
        ...
        if ch == 'do':
          ...
          if target_list is do:
            if len(do) == 0:
              sys.exit(_('Error in rule file: %s - a [donot] section without a corresponding [do] section.') % filename)
          elif len(donot) > 0:
            sys.exit(_('Error in rule file: %s - two consecutive [donot] sections.') % filename)
          target_list = donot
        else:
          sys.exit('Internal error: unfiltered malformed_header.')
      elif not block.strip():
        sys.exit(_('Empty section in rule file: %s') % filename)
      elif not efabrules.process_rules(filename, block.strip(), target_list,
                                    clean_before=False, context_allows_substitutions=target_list is do):
        if not args.permissive:
          sys.exit(_('Aborting due to error(s) found while processing rule file: %s.') % filename)
    rulesets.append(ruleset(do, donot))
  ...
  try:
    with openwriter() as outfile:
      with openreader() as infile:
        text = codecs.decode(infile.read(), encoding=input_encoding, errors='replace')
        for rs in rulesets:
          text = process_ruleset(rs, text)
        outfile.write(codecs.encode(text, encoding=output_encoding, errors='replace'))
  except IOError as e:
...
def process_ruleset(ruleset, text):
  ...
  for rule in ruleset.do:
    try:
      # Create a set spanning all positions within the text which are protected by 'do not' rules
      if recalculate_protected_areas:
        ...
        for protection_rule in ruleset.donot:
          for m in protection_rule.regex_object.finditer(text):
            ...
            # E.g., if a match spans the range (10, 15), this line will add {10, 11, 12, 13, 14} to protected_area
            protected_area.update(range(*protected_span))
      # Apply the substitution rule to unprotected areas in the text
      # If a substitution was actually performed, delete protected_area, so it is updated in next iteration
      recalculate_protected_areas = False
      text = efabrules.sub(rule, text, repl=lambda m: replace_if_not_protected(rule, m, protected_area))
...
def log_protection_rule(rule, span, m):
  if args.log:
    ...
    print(_('Protected text: %s') % repr(m.group()).strip("'"), file=logfile)
    print(file=logfile)
...
def log_substitution_rule_not_applied():
  if args.log:
    print(_('The rule was not applied because the target span intersects a protected area.'), file=logfile)
    print(file=logfile)
