...
class rule(object):
  def __init__(self, filename, definition, regex_object, pattern, repl, allow_extended_repl=True):
    ...
    if allow_extended_repl and re.search(EXTENDED_REPL, repl):
      self._repl = lambda m: self._expand_extended_replacement(m, re.split(EXTENDED_REPL, repl))
    else:
...
def _report_wrong_rule(filename, definition, why):
  ...
  print(_('Wrong rule in file: %s. Reason is: %s.\n') % (filename, why), file=sys.stderr)
  print(definition, file=sys.stderr)
  print(file=sys.stderr)
  errors_detected = True


def process_rules(filename, content, rule_list, clean_before=True, context_allows_substitutions=True):
  global errors_detected

  if clean_before:
    content = content.split('\n')
    content = list(map(str.strip, content))
    content = [x for x in content if not x or x[0] != '#'] # Delete comment lines
    content = '\n'.join(content).strip()
  if not content:
    return False
  errors_detected = False
  for block in re.split(r'\n{2,}', content):
    lines = block.split('\n')
    if len(lines) == 1:
      _process_one_line_block(filename, lines[0], rule_list, context_allows_substitutions)
...
RULE_MUST_BE_IN_12LINE_BLOCK  = _('simple-match and plain text rules must be used in one- or two-line block')
RULE_AMBIGUOUS_TWO_LINE_BLOCK = _('two-line block is ambiguous (missing separating new line?)')
RULE_WRONG_TWO_LINE_BLOCK     = _('two-line block’s second line must be plain text')
...
def _process_substitution_rule(filename, line, rule_list):
  if efabregex.is_substitution(line):
    if efabregex.contains_invalid_flags(line):
      ...
      try:
        regex_object, pattern, repl = efabregex.compile_substitution(line)
    ...
    if efabregex.contains_invalid_flags(line):
      # Perhaps trying to create a substitution rule and omitted the closing delimiter
    ...
    # No chance to interpret the line as a regex, it looks more like a plain text rule
    _report_wrong_rule(filename, line, RULE_MUST_BE_IN_12LINE_BLOCK)


def _process_one_line_block(filename, line, rule_list, context_allows_substitutions):
  regex = None
  ...
  if regex:
    try:
      ...
      new_rule = rule(filename, line, regex_object, pattern, _default_replacement)
      rule_list.append(new_rule)
    except efabregex.error as e:
      _report_wrong_rule(filename, line, e)
  else:
...
def _process_two_line_block(filename, block, lines, rule_list):
  if efabregex.is_substitution(lines[0]):
    ...
    # The second line can only be plain text (as required) because of the test immediately above.
    ...
    if efabregex.is_pattern(lines[0]):
      if efabregex.contains_invalid_flags(lines[0]):
        ...
        return
      regex = lines[0]
    else: # It is neither a pattern nor a substitution (first test above); it can only be plain text
      regex = efabregex.create_match(re.escape(lines[0]))
    try:
      regex_object, pattern = efabregex.compile(regex)
      new_rule = rule(filename, block, regex_object, pattern, repl, allow_extended_repl=False)
      rule_list.append(new_rule)
    except efabregex.error as e:
      _report_wrong_rule(filename, block, e)


RULE_NOT_APPLIED = _('The rule is wrong and was not applied: %s\n\nThe reason is: %s')


def sub(rule, text, repl=None):
  repl = repl or rule._repl # use _repl, so extended replacements will be applied
  if repl is None:
    sys.exit('Internal error while processing rule %s - no replacement given.' % rule.definition)
  try:
