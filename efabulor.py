...
# Import needed Python packages. I mostly prefer to import the module as a whole (instead of from module import ...)
# to avoid messing with the global namespace
...
"""FOR TRANSLATORS: This is the key users should press to answer YES to a yes/no question"""
YES_KEY = _('y')
"""FOR TRANSLATORS: This is the key users should press to answer NO to a yes/no question"""
NO_KEY = _('n')
...
def staticclass(cls):
  ...
  cls.__init_subclass__ = lambda *args: raise_exc('This class is unsubclassable by design.')
  return cls

def mainthreadmethod(func): #TODO add the assertion directly to the relevant methods and eliminate this decorator?
  def decorated(*args, **kwargs):
    if threading.current_thread() is not threading.main_thread():
      caller_name = func.__qualname__ + '()'
      Main.terminate('Internal error: calling %s from outside the main thread is prohibited by design.' % caller_name)
...
class Output:
  ...
  def window_width(cls, target=sys.stdout):
    try:
      return os.get_terminal_size(target.fileno()).columns - RuntimeOptions.window_width_adjustment()
    except OSError:
      return cls.DEFAULT_WINDOW_WIDTH - RuntimeOptions.window_width_adjustment()
  ...
  def say(cls, what, type_of_msg, wrap=True, print_prompt=True, end='\n\n'):
    ...
    if not isinstance(what, str):
      Main.terminate('Internal error: called Output.say() with a non-string')
    ...
    with cls._lock:
      ...
      if wrap:
        ...
        if type_of_msg== cls.NORMAL:
          width -= RuntimeOptions.left_indent() + RuntimeOptions.right_indent()
        what = '\n'.join(textwrap.fill(x, width, break_on_hyphens=False, replace_whitespace=False)
                          for x in what.split('\n'))
        if type_of_msg == cls.NORMAL:
          what = textwrap.indent(what, ' ' * RuntimeOptions.left_indent())
  ...
  def report_error(cls, e, type_of_msg=None):
    with cls._lock:
      if isinstance(e, subprocess.CalledProcessError):
        cls.say(_('An error has occurred while executing an external command %s.') % e.cmd, type_of_msg=cls.ERROR)
        cls.say(_('The process error output is:'), type_of_msg=cls.ERROR)
        cls.say(e.stderr.strip(), type_of_msg=cls.ERROR_EXTENDED)
      if type_of_msg is None:
        type_of_msg = cls.ERROR_EXTENDED if isinstance(e, EspeakControllerError) else cls.ERROR
  ...
  _HELPLESS = _('?eEOF:%lt-%lb.\: Use the arrow keys to scroll or press q to return to the program.')
  ...
  def pager(cls, what, title=None, wrap=False):
    ...
    if wrap:
      what = what.split('\n')
      what = map(lambda x: textwrap.fill(x, cls.window_width(), break_on_hyphens=False), what)
      what = '\n'.join(what)
    with cls._lock:
      if title:
      ...
      if WINDOWS:
        try:
          codepage = subprocess.run(['mode', 'con', 'cp'], shell=True, capture_output=True).stdout
          codepage = int([x for x in codepage.split() if x.isdigit()][0])
          what = what.encode('cp%s' % codepage, 'backslashreplace')
        except Exception as e:
          cls.report_error(_('An error has occurred while trying to convert the log to the console’s encoding.'))
          cls.say(_('The output might contain wrong characters.'), type_of_msg=cls.INFO)
        subprocess.run('more', shell=True, input=what, text=isinstance(what, str), check=True)
        print()
      else:
        subprocess.run(['less', '-Ps%s' % cls._HELPLESS], input=what, text=True, check=True)
...
class EspeakController:
  ...
  def say(self, what, on_start=None):
    with self._lock:
      if self._running:
        raise EspeakControllerError('Internal error: trying to start an already running player.')
      try:
      ...
      if errors:
        ...
        if self._espeak.returncode and self._running:
          ...
          Output.report_error(errors, type_of_msg=Output.ERROR_EXTENDED)
      self._running = False
  ...
  def toggle(self):
    with self._lock:
      if self._running and self._espeak_still_running():
        ...
        if LINUX:
          self._espeak.send_signal(signal.SIGSTOP if self._paused else signal.SIGCONT)
          return True
        elif WINDOWS:
          if PSUTIL_INSTALLED:
            if self._paused:
          ...
          try:
            if self._paused:
              subprocess.run([PSSUSPEND, str(self._espeak.pid)], check=True, stdout=subprocess.DEVNULL,
                             stderr=subprocess.DEVNULL)
            else:
              subprocess.run([PSSUSPEND, '-r', str(self._espeak.pid)], check=True, stdout=subprocess.DEVNULL,
                             stderr=subprocess.DEVNULL)
            return True
          except:
            self._paused = not self._paused
...
class Main:
  ...
  if LINUX:
    signal.signal(signal.SIGHUP, lambda signum, frame: Main.terminate(Main.TERMINATED_BY_SIGNAL % 'SIGHUP'))
  ...
  def run_safely(cls, func):
    try:
      if func is not None: # Perhaps it could be None during interpreter shutdown?
        func()
    except Exception as e:
      Output.say(_('An uncaught exception has occurred: %s.') % e, type_of_msg=Output.ERROR)
  ...
  #If you add new event types here, you can assign them any value, but be careful to ensure no values are repeated.
  ...
  WILL_CONTINUE = _('The player will stop now. ') + \
    _('If you restart the player, the program will continue as if the text was not modified.')
    ...
    while True:
      ...
      if not cls._event_queue.empty():
        ...
        if evt == cls.INPUT_FILE_CHANGED:
          if Player.running() or RuntimeOptions.reload_when_not_playing():
          ...
          text, lines = data
          if not text or not lines: # This should never happen, but we add the extra check for the sake of robustness.
          ...
          if not Player.text_is_loaded():
            ...
            Player.stop()
        elif evt in [cls.LINE_READING_ENDED, cls.SPOKEN_FEEDBACK_ENDED]:
          # Here data contains a function call that the generator of the events wants to have executed in the main thread.
          data()
        elif evt == cls.TERMINATION_MESSAGE:
          # This is from a previous implementation of Main.terminate(), and should never execute, but we leave it just in case
          # someday we want to reimplement termination by event.
          cls.terminate(data)

  @classmethod # class Main
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _schedule_delayed_reload(cls):
    Output.say(_('The input file will be reloaded upon restart.'), type_of_msg=Output.INFO)
  ...
  # Tampering with the following method can break the @mainthreadmethod decorator, and in fact, the
  ...
  def terminate(cls, msg=None):
    if threading.current_thread() is threading.main_thread():
      ...
      sys.exit() # This line is needed to terminate the calling thread, do not remove it.
...
class Player:
  ...
  def line_number(cls, value=None, showline=True):
    ...
    if value != cls._line_number and \
        RuntimeOptions.stop_after_current_line() and \
  ...
  def line_to_be_read_changed(cls):
    ...
    cls._line_to_be_read = Substitutions.apply(line) if RuntimeOptions.apply_subst() else line
  ...
  def substitution_rules_changed(cls):
    ...
    if cls.line_to_be_read_changed():
      ...
      if a or b:
        ...
        Output.say(_('New substitutions will be applied when reading is restarted.'), type_of_msg=Output.INFO)
    else:
      Output.say(_('New substitution rules do not affect this line.'), type_of_msg=Output.INFO)
  ...
  def _say_line(cls, line):
    ...
    try:
      ...
      with Output.get_lock():
        Output.say(_('An error has occurred while trying to run espeak.'), type_of_msg=Output.ERROR)
        Output.report_error(e)
        Output.say(_('The reading is stopped, you can try to restart it.'), type_of_msg=Output.INFO)
    ...
    with cls._lock:
      if not cls._running:
        ...
        cls._running = False
        cls._started.set() # In case .say() didn't set it and start() is waiting.
      else:
  ...
  def _continue(cls):
    ...
    if sequence_mode.eof():
      if RuntimeOptions.close_at_end() and sequence_mode is SEQUENCE_NORMAL:
        cls._lock.release()
        Main.terminate()
      else:
        cls._at_eol = True
        cls._running = False
        cls._lock.release()
    elif RuntimeOptions.stop_after_current_line() or RuntimeOptions.stop_after_each_line():
      cls._at_eol = True
      cls._running = False
      cls._lock.release()
      with Output.get_lock():
  ...
  def reset_pointer(cls, say_it=True):
    if cls._at_eol:
      cls._at_eol = False
      if say_it:
        Output.say(_('The player was reset to the beginning of the current line.'), type_of_msg=Output.INFO)
  ...
  def toggle(cls):
    with cls._lock:
      if cls._running:
        ...
        if cls._text_player.running():
          if not cls._text_player.toggle(): # Can be False in Windows
            cls._paused = not cls._paused
        elif not cls._paused:
          cls._continue()
        return
      # If we are at the end of the file, then _at_eol can be True. In that case, let's just read
      # the last line again.
  ...
  def showline(cls, requested_by_user=False):
    ...
    if RuntimeOptions.no_echo() and not requested_by_user:
      return False
    t = cls._line_to_be_read if RuntimeOptions.show_subst() else cls._lines[cls._line_number]
    if RuntimeOptions.show_line_number():
      lineno = str(cls._line_number + 1)
      tot = ('/' + str(len(cls._lines))) if RuntimeOptions.show_total_lines() else ''
      t = '<%s%s> %s' % (lineno, tot, t)
    # If show_line() gets called again to print the same line, and nothing else has been printed by Output.say()
    # since the last call from show_line(), do not print the same line again, unless requested_by_user is True.
    # CAUTION: do not try to simplify by setting a variable this_call = (cls._line_number, t, Output.get_counter())
    # here and assigning it to _last_shown_line below, because the output counter will get incremented after a
    # successful call to Output.say().
    if (cls._line_number, t, Output.get_counter()) == cls._last_shown_line and not requested_by_user:
      return False
    if Output.say(t, type_of_msg=Output.NORMAL):
      # If the call to Output.say() is successful, update _last_shown_line.
...
class Commands:
  ...
  # (i.e., before the actual definition of functions) without breaking it.
  _bindings_player = {
  ...
  _bindings_general = {
    ...
    ASK_N_QUIT_CMD  : lambda:    NonPlayerCommands.quit(ask_for_confirmation=True),
  ...
  def _parse_simple(cls, cmd):
    ...
    if not args and verb in cls._commands_with_no_args + cls._commands_with_or_without_args:
      return cmd
    ...
    if verb == 'sh':
      ...
      if len(args) > 3:
        return None
      if len(args) >= 2 and args[1] not in [PlayerCommands.SEARCH_CI, PlayerCommands.SEARCH_CS]:
        return None
      if len(cmd) >= 1 and args[0] not in [PlayerCommands.SEARCH_PLAIN, PlayerCommands.SEARCH_REGEX]:
        return None
  ...
  def _process_simple(cls, cmd):
    ...
    try:
      ...
      Output.say(cls.UNSUCCESFUL_COMMAND % (cmd, e), type_of_msg=Output.ERROR)
      return False
...
class PlayerCommands:
  ...
  def go_line(line_number=None):
    with Output.get_lock():
      ...
      if line_number < 1 or line_number > len(Player.lines()):
        Output.say(_('You must enter a line number between 1 and %s.') % len(Player.lines()), type_of_msg=Output.ERROR)
        return False
      elif Player.line_number() == line_number-1:
        Output.say(_('The player is already at line %s.') % line_number, type_of_msg=Output.INFO)
        return True
      else:
  ...
  MSG_ENTER_REGEX = _('Enter a regular expression, without delimiters (press Enter to cancel):')
  ...
  def find(cls, mode=None, cs=None, what=None):
    ...
    with Output.get_lock():
      if not mode:
        mode = UserInput.choose_mode(_('Choose a search mode:'), [cls.SEARCH_PLAIN, cls.SEARCH_REGEX], None)
        if not mode:
          return False
        Output.say(_('Search mode is: %s') % mode, type_of_msg=Output.INFO)
      show_cs = True
      if not cs:
        cs = UserInput.choose_mode(_('Choose a case sensitivity mode:'), [cls.SEARCH_CI, cls.SEARCH_CS], None)
        if not cs:
          return False
      ...
      if not what:
        if show_cs:
          Output.say(cls.MSG_CASE_SENS % cs, type_of_msg=Output.INFO)
        Output.say(cls.MSG_ENTER_PLAIN if mode==cls.SEARCH_PLAIN else cls.MSG_ENTER_REGEX, type_of_msg=Output.INTERACTION)
      ...
      try:
        _flags = (efabregex.get_default_flags() & ~re.IGNORECASE) | (re.IGNORECASE if cs==cls.SEARCH_CI else 0)
        cls._find_re = re.compile(re.escape(cls._find_what) if mode==cls.SEARCH_PLAIN else cls._find_what, flags=_flags)
        return cls._find_next(0)
      except re.error as e:
        Output.report_error(e)
        Output.say(_('The search was cancelled because the pattern was wrong'), type_of_msg=Output.INFO)
        return False
  ...
  FOUND_SAME = _('A match was found for the search expression (%s) at the current line (%s).')
    ...
    for i in range (where_from, len(Player.lines())):
      if cls._find_re.search(Player.lines()[i]):
        if i == Player.line_number():
          Output.say(cls.FOUND_SAME % (cls._find_what, i+1), type_of_msg=Output.INFO)
        else:
          Output.say(cls.FOUND % (cls._find_what, i+1), type_of_msg=Output.INFO)
  ...
  def change_speed(delta):
    with EspeakController.get_lock():
      ...
      if speed >= MAXSPEED and delta > 0:
        Output.say(_('Already at maximum speed'), type_of_msg=Output.INFO)
        return False
      elif speed <= MINSPEED and delta < 0:
        Output.say(_('Already at minimum speed'), type_of_msg=Output.INFO)
        return False
      speed += delta
...
class NonPlayerCommands:
  ...
  def run_in_shell(cls, cmd=None):
    ...
    if not cmd:
      ...
      with Output.get_lock():
        subprocess.run(cmd, shell=True, stdout=Output.get_file(Output.NORMAL), stderr=Output.get_file(Output.NORMAL))
  ...
  def open_shell():
    ...
    with Output.get_lock():
      Output.say(_('Entering the shell; close the shell to return to the program.'), type_of_msg=Output.INFO)
      ...
      try:
        ...
        ret = False
        Output.report_error(e)
        Output.say(_('The program could not open a shell.'), type_of_msg=Output.ERROR)
      return ret
  ...
  def open_file(cls, name):
    if not os.path.exists(name):
      Output.say(_('The file/directory %s does not exist.') % name, type_of_msg=Output.ERROR)
      return False
    else:
  ...
  def _open_file(name):
    try:
      ...
      Output.say(_('Opening %s in another window.') % name, type_of_msg=Output.INFO)
      # In case you modify the following call, be VERY careful to ensure sanitization.
      if WINDOWS:
        subprocess.run(['start', name], shell=True, text=True, capture_output=True, check=True)
      else:
        subprocess.run(['xdg-open', name], text=True, capture_output=True, check=True)
    except Exception as e:
      with Output.get_lock():
        Output.report_error(e)
        Output.say(_('The program could not open file: %s') % name, type_of_msg=Output.ERROR)
  ...
  @classmethod # class NonPlayerCommands
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def choose_and_open_file(cls, file_list, no_file_msg=_('There are no files to edit.')):
    ...
    with Output.get_lock():
      Output.say(_('Choose file to edit:'), type_of_msg=Output.INTERACTION)
  ...
  def show_input_command():
    Output.say(_('The input command is:\n\n%s') % InputTextLoader.input_command(), type_of_msg=Output.INFO)
  ...
  def command_loop(cls):
    if cls._in_command_loop:
      Output.say(_('The petition to enter the command loop was ignored because you are already in the command loop.'), type_of_msg=Output.INFO)
      return
    cls._in_command_loop = True
    msg = _('You have entered the command line mode.')
    msg += '\n' + _('Here you can enter one or more commands in the internal scripting language.')
    ...
    with Output.get_lock():
      ...
      while True:
        line = UserInput.readline(prompt=Output.get_prompt(Output.INTERACTION))
        Output.separate(Output.INTERACTION)
        if not line:
          Output.say(_('You are returning to the normal mode.'), type_of_msg=Output.INFO)
          break
  ...
  # No matter how wonderful this program is, the user will want to get out from it sooner or later.
  @staticmethod # class NonPlayerCommands
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def quit(ask_for_confirmation=True):
    if ask_for_confirmation and not RuntimeOptions.quit_without_prompt() and not Main.scripted_mode:
      with Output.get_lock():
        if not UserInput.confirm_action(_('Do you really want to quit?')):
          return False
    Main.terminate()
...
class SpokenFeedback:
  ...
  def get_message(cls, key):
    if key in cls._messages and cls._message_levels[key] <= RuntimeOptions.feedback_mode():
...
class InputTextLoader:
  ...
  def input_command(cls, value=None):
    with cls._lock:
      ...
      # _input_command was already sanitized by CmdLineArgs._get_input_source(), except for --do and --preprocess.
      # We convert it into a series of separate commands, so we can check the exit status of each one.
      try:
        cls._input_pipeline = cls._create_input_pipeline(cls._input_command)
      except ValueError as e:
        Main.terminate(_('A wrong input command was given: %s. Reported error is: %s.') % (value, e))
  ...
  def log_transformation_rules(cls):
    with cls._lock:
      if not cls._transformation_log_command:
        Output.say(_('Cannot show the effect of transformation rules because none were given.'), type_of_msg=Output.ERROR)
        return
      if not cls._transformation_log_pipeline:
        # _transformation_log_command was already sanitized by CmdLineArgs._get_transformation_rules().
        cls._transformation_log_pipeline = cls._create_input_pipeline(cls._transformation_log_command)
      try:
        Output.say(_('Preparing the transformation-rules log.'), type_of_msg=Output.INFO)
        text = None
        # _transformation_log_pipeline should be already sanitized (see above). Be VERY careful to ensure sanitization if you
        # were to modify the following code or the code in CmdLineArgs on which it depends.
        for cmd in cls._transformation_log_pipeline:
          cp = subprocess.run(cmd, input=text, text=True, check=True, capture_output=True)
          text = cp.stdout
        log = cp.stderr
        if len(log) > 0:
          Player.stop()
          if Main.scripted_mode:
            ...
            Output.pager(log, title=_('Showing the transformation-rules log.'))
        else:
          Output.say(_('It seems the text source was not affected by the transformation rules.'), type_of_msg=Output.INFO)
      except Exception as e:
        with Output.get_lock():
          Output.report_error(e)
          Output.say(_('The program cannot show the transformation-rules log.'), type_of_msg=Output.ERROR)
  ...
  ERROR_GETTING_INPUT = _('An error has occurred while trying to read the text source.')
  NO_READABLE_TEXT = _('The text source is empty or contains no readable text.')
    ...
    with cls._lock:
      if cls._input_file is not None:
        if not os.path.isfile(cls._input_file):
          Output.say(cls.CANNOT_FIND_FILE % cls._input_file, type_of_msg=Output.INFO)
          Main.event(Main.TEXT_LOADING_ERROR)
          cls._running = False
          cls._stopped.notify()
          return
        else:
          Output.say(_('Reading file: %s ...') % cls._input_file, type_of_msg=Output.INFO)
      else:
        Output.say(_('Reading output from: %s') % cls._input_command, type_of_msg=Output.INFO)
      try:
        text = None
        # _input_pipeline should be already sanitized (see InputTextLoader.input_command()).
        for cmd in cls._input_pipeline:
          cls._lock.release()
          process = subprocess.Popen(cmd, encoding=INPUT_ENCODING_SIG,
          ...
          if errors:
            with Output.get_lock():
              ...
              Output.say(cls.ERROR_GETTING_INPUT, type_of_msg=Output.ERROR)
            Main.event(Main.TEXT_LOADING_ERROR)
            cls._running = False
            cls._stopped.notify()
            return
  ...
  def _split_text_and_create_event(cls, text):
    if not text.strip():
      ...
      if not lines:
        ...
        if InputTextLoader.segmenting_regex() or InputTextLoader.separator_regex():
          msg += ' ' + _('Hint: perhaps the segmentation rule or separator given in the command line is wrong.')
        Output.say(msg, type_of_msg=Output.ERROR)
        Main.event(Main.TEXT_LOADING_ERROR)
        return
...
class FileMonitor:
  ...
  MONITORING_ERROR = _('Error while trying to get modification time of file/directory: %s')
  WRONG_MONITORED_TARGET = _('%s is not a file nor a directory. Its modification time cannot be checked.')
    ...
    with cls._lock:
      ...
      for f in cls._files:
        try:
          ...
          Output.say(cls.MONITORING_ERROR % f, type_of_msg=Output.ERROR)
    ...
    for group, action in files_to_check:
      ...
      for f in group:
        if not checking_msg_already_printed and say_it:
          Output.say(_('Checking modification times of monitored files/directories.'), type_of_msg=Output.INFO)
        ...
        if os.path.isfile(f) or os.path.isdir(f):
          ...
          Output.say(cls.WRONG_MONITORED_TARGET % f, type_of_msg=Output.ERROR)
      if change_detected:
        action()
      elif say_it and not no_changes_msg_already_printed:
        Output.say(_('No modification time changes were detected.'), type_of_msg=Output.INFO)
    ...
    with cls._lock:
      for line in cls._spoken_feedback:
        try:
          ...
          cls._lock.acquire()
          with Output.get_lock():
            Output.say(_('There was an error while reading spoken feedback.'), type_of_msg=Output.ERROR)
            Output.report_error(e)
            Output.say(_('The reading is stopped, you can try to restart it.'), type_of_msg=Output.INFO)
          cls._running = False
          return
...
class UserInput:
  ...
  # This class does not implement a lock, as all of its methods MUST only be called from the main thread.

  _old_tty_settings = None

  if LINUX:
    signal.signal(signal.SIGCONT, lambda signum, frame: UserInput.set_normal())
  ...
  def set_normal(cls):
    ...
    if cls._old_tty_settings is not None:
      termios.tcsetattr(sys.stdin.fileno(), termios.TCSANOW, cls._old_tty_settings)
  ...
  def getch(cls, timeout=0):
    ...
    try:
      if timeout and not cls._input_available(timeout):
        return None
      if LINUX:
        return str(os.read(sys.stdin.fileno(), cls._A_LARGE_ENOUGH_NUMBER), encoding=SYSTEM_ENCODING)
      elif WINDOWS:
  ...
  def get_int(cls):
    ...
    with Output.get_lock():
      ...
      if reply.isdigit():
        ...
        Output.say(_('You must enter a positive integer.'), type_of_msg=Output.ERROR)
        Output.report_action_cancelled()
        return None
    ...
    with Output.get_lock():
      ...
      while True:
        ...
        if ch in [NO_KEY.lower(), YES_KEY.lower()]:
          Output.say(ch, type_of_msg=Output.INTERACTION, print_prompt=False)
          answer = (ch == YES_KEY)
          break
        elif ch in [chr(13), chr(10)]:
          Output.say(NO_KEY.lower(), type_of_msg=Output.INTERACTION, print_prompt=False)
          break
      return answer
...
class SEQUENCE_RANDOM:
  ...
  msg_first = _('Jumping back to the beginning of the random reading history.')
...
class KeyBindings:
  ...
  def get_parsed_command(cls, ch):
    if not ch in cls._parsed_bindings:
  ...
  def is_bound_to_quit_command(cls, ch):
    if not ch in cls._key_bindings_to_quit:
      parsed_command = cls.get_parsed_command(ch)
      if not parsed_command:
        return False
      cls._key_bindings_to_quit[ch] = Commands.contains_quit(parsed_command)
...
class Substitutions:
  ...
  def show_log(cls):
    if not cls._rules:
      Output.say(_('Cannot show the effect of substitution rules because none were given.'), type_of_msg=Output.INFO)
      return
    if not RuntimeOptions.apply_subst():
      Output.say(_('Substitution rules are not being applied.'), type_of_msg=Output.INFO)
      return
    if len(cls._hist) == 1:
      Output.say(_('No substitutions were applied to this line.'), type_of_msg=Output.INFO)
      return
    s = ''
    ...
    if Main.scripted_mode:
      ...
      try:
        Output.pager(s, title=_('Showing the substitution history.'), wrap=True)
      except Exception as e:
        with Output.get_lock():
          Output.report_error(e)
          Output.say(_('The program cannot show the substitution history.'), type_of_msg=Output.ERROR)
...
class RuntimeOptions:
  ...
  # Validators for options that are currently being managed by RuntimeOptions directly are defined here.
  # When an option is managed by an instance of an option class (see below), that instance will add a validator to this dictionary.
  # TODO: if someday we convert 'static' classes into singleton objects, option classes might implement the full descriptor protocol.
  _validators = {'apply-subst': lambda x: x in RuntimeOptions._BooleanOption._true_values + \
                                               RuntimeOptions._BooleanOption._false_values,
                  'show-subst': lambda x: x in RuntimeOptions._BooleanOption._true_values + \
                                               RuntimeOptions._BooleanOption._false_values,
                  'no-info': lambda x: x in RuntimeOptions._BooleanOption._true_values + \
                                            RuntimeOptions._BooleanOption._false_values,
                  'monitoring-interval': lambda x: x.isdigit(),
                  'tracking-mode': lambda x: x in RuntimeOptions.TRACKING_OPTIONS,
                  'sequence-mode': lambda x: x in RuntimeOptions.SEQUENCE_OPTIONS,
                  'feedback-mode': lambda x: x in RuntimeOptions.FEEDBACK_OPTIONS
                  }
  ...
  class _BooleanOption:
    ...
    def __set_name__(self, owner, name):
      if not re.match(r'[a-z][a-z_]*[a-z]$', name):
        raise AttributeError('Internal error in RuntimeOptions: invalid option name %s.' % name)
      scripting_name = name.replace('_', '-')
      owner._validators[scripting_name] = lambda x: x in self._true_values + self._false_values
    ...
    def _scripting_setter(self, value=None, say_it=False):
      ...
      value = (not old_value) if value is None else (value in self._true_values)
  ...
  class _IntervalOption:
    ...
    def __set_name__(self, owner, name):
      if not re.match(r'[a-z][a-z_]*[a-z]$', name):
        raise AttributeError('Internal error in RuntimeOptions: invalid option name %s.' % name)
      scripting_name = name.replace('_', '-')
      owner._validators[scripting_name] = lambda x: x.isdigit() and int(x) >= 0
      owner._scripting_setters[scripting_name] = self._scripting_setter
      setattr(owner, name, staticmethod(self._public_accessor))
    ...
    def _get_pause_value():
      with Output.get_lock():
        Output.say(_('Enter the length of the pause in seconds:'), type_of_msg=Output.INTERACTION)
  ...
  no_showline = _BooleanOption(False,
                               _('The current line will not be printed even if requested by the user.'),
                               _('The current line will be printed when requested by the user.'))

  no_echo = _BooleanOption(False,
                           _('The current line will not be printed when the line number changes.'),
                           _('The current line will be printed when the line number changes.'))

  no_update_player = _BooleanOption(False,
                                    _('The player will not be restarted if the line number changes while reading.'),
                                    _('The player will be restarted if the line number changes while reading.'))

  show_line_number = _BooleanOption(False,
                                    _('Line numbers will be printed.'),
                                    _('Line numbers will not be printed.'))

  show_total_lines = _BooleanOption(False,
                                    _('Number of total lines will be printed.'),
                                    _('Number of total lines will not be printed.'))

  stop_after_current_line = _BooleanOption(False,
                                    lambda: _('The player will stop after reading the current line.') \
                                      if Player.running() else _('The player will stop after reading the next line.'),
                                    lambda: _('The player will not stop after reading the current line.') \
                                      if Player.running() else _('The player will not stop after reading the next line.'))

  reset_scheduled_stop_after_moving = _BooleanOption(True,
  ...
  stop_after_each_line = _BooleanOption(False,
                                        _('The player will stop after reading each line.'),
                                        _('The player will not stop after reading each line.'))

  restart_after_change = _BooleanOption(True,
                                        _('The player will restart when changes to input files are detected.'),
                                        _('The player will not restart when changes to input files are detected.'))

  restart_after_substitution_change = _BooleanOption(False,
                                        _('The player will restart when changes to the substituted line are detected.'),
                                        _('The player will not restart when changes to the substituted line are detected.'))

  restart_on_touch = _BooleanOption(False,
                                _('Modification time updating without a content change will be enough to restart the player.'),
                                _('Modification time updating without a content change will not be enough to restart the player.'))

  restarting_message_when_not_playing = _BooleanOption(True,
    _('Spoken feedback will be given when the player is not playing and the reading is restarted in the same line.'),
    _('Spoken feedback will not be given when the player is not playing and the reading is restarted in the same line.'))

  reload_when_not_playing = _BooleanOption(True,
                                           _('The text source will be reloaded when changes to monitored files are detected.'),
                                           _('The text source will not be reloaded when changes to monitored files are detected.'))

  close_at_end = _BooleanOption(False,
  ...
  quit_without_prompt = _BooleanOption(False,
                                       _('The program will not ask for confirmation when asked to quit.'),
                                       _('The program will ask for confirmation when asked to quit.'))

  pause_before = _IntervalOption(0,
                                 lambda x: _('The reader will pause %s second(s) before starting to read when stopped.') % x)

  pause_between = _IntervalOption(0,
                                  lambda x: _('The reader will pause %s second(s) between lines.') % x)

  left_indent = _IntervalOption(0,
                                  lambda x: _('A left indent of %s space(s) will be used.') % x)

  right_indent = _IntervalOption(0,
                                  lambda x: _('A right indent of %s space(s) will be used.') % x)

  window_width_adjustment = _IntervalOption(0,
                                  lambda x: _('%s space(s) will be added to the right indent.') % x)

  # Options that don't fit the option classes' framework well (yet). # TODO: perhaps we can make them to?

  # We let class Output manage no_info by itself, because it might need to check it from outside the main thread.
  # (In this version of RuntimeOptions, access to options is reserved for the main thread, and unlocked. We might
  ...
  def no_info(cls, value=None, say_it=False):
    with Output.get_lock():
      ...
      if say_it:
        ...
        if value:
          Output.say(_('Informative messages will not be printed.'), type_of_msg=Output.INFO)
        else:
          Output.say(_('Informative messages will be printed.'), type_of_msg=Output.INFO)
      Output.no_info(value)

  NO_SUBST = _('The option cannot be set because no substitution rules were given.')
  ...
  def show_subst(cls, value=None, say_it=False):
    ...
    if not cls._apply_subst:
      if say_it:
        Output.say(_('The option cannot be set because substitutions are not being applied.'), type_of_msg=Output.INFO)
      return
    ...
    if say_it:
      if value:
        Output.say(_('The effect of substitution rules will be shown instead of the actual text.'), type_of_msg=Output.INFO)
      else:
        Output.say(_('The effect of substitution rules will not be shown instead of the actual text.'), type_of_msg=Output.INFO)

  # We let class FileMonitor manage monitoring_interval by itself, because it needs to check it from outside the main thread.
  # (In this version of RuntimeOptions, access to options is reserved for the main thread, and unlocked. We might
  # want to change that in future versions.)
  ...
  def monitoring_interval(cls, value=None, say_it=False):
    ...
    if say_it:
      Output.say(_('Files will be checked for changes each %s second(s).') % value, type_of_msg=Output.INFO)

  FEEDBACK_OPTIONS = {'none': SpokenFeedback.NO_FEEDBACK,
  ...
  @classmethod # class RuntimeOptions
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _set_tracking_mode(cls, to_what=None, say_it=False, adjust_sequence_mode=True):
  ...
  def _set_tracking_mode_aux(cls, name, say_it, adjust_sequence_mode):
    if name is None:
      UserInput.choose_mode(_('Choose a tracking mode:'),
      ...
      if say_it:
        Output.say(_('Tracking mode set to: %s.') % name, type_of_msg=Output.INFO)
      if adjust_sequence_mode and cls._sequence_mode is not SEQUENCE_NORMAL:
        cls._set_sequence_mode(SEQUENCE_NORMAL, say_it, adjust_tracking_mode=False)
    else:
  ...
  @classmethod # class RuntimeOptions
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _set_sequence_mode(cls, to_what=None, say_it=False, adjust_tracking_mode=True):
  ...
  def _set_sequence_mode_aux(cls, name, say_it, adjust_tracking_mode):
    if name is None:
      UserInput.choose_mode(_('Choose a sequence mode:'),
  ...
  def _set_feedback_mode_aux(cls, name, say_it):
    if name is None:
      UserInput.choose_mode(_('Choose a feedback mode:'),
      ...
      if say_it:
        Output.say(_('Feedback mode set to: %s.') % name, type_of_msg=Output.INFO)
    else:
      # It will be caught by process_command
      raise AttributeError()
...
class CmdLineArgs:
  ...
  def process(cls):
    ...
    WRONG_INT = _('The %s option must be an integer greater than or equal to zero.')
    ...
    # Default regex flags must be set before calling other code which might depend on them.
    flags = args.regex_flags.strip().replace(' ', '')
    if flags:
      try:
        efabregex.set_default_flags(flags)
      except efabregex.error:
        Main.terminate(_('Invalid flags for regular expressions: %s.') % flags)
    ...
    InputTextLoader.transformation_rule_files(cls._transformation_rule_files)
    InputTextLoader.transformation_log_command(cls._transformation_log_command)
    cls._get_command_line_monitored_files(args)

    FileMonitor.register_group(cls._monitored_files, lambda: Main.event(Main.INPUT_FILE_CHANGED))

    cls._set_substitution_rules(args)
    cls._set_runtime_options(args)

    Output.say(_('Encoding used for input: %s') % INPUT_ENCODING, type_of_msg=Output.INFO)
  ...
  def _get_args(cls):
    ...
    parser.add_argument('-c', '--config-file', metavar=_('<configuration file for voice and feedback messages>'))
    parser.add_argument('-C', '--text-conversion-config',
                        metavar=_('<configuration file for text conversion> %s') % cls._SECURITY_WARNING)
    parser.add_argument('-v', '--voice', metavar=_('<name of the voice to be used>'))
    parser.add_argument('-p', cls.PAUSE_BEFORE_SWITCH, nargs='?', const=3, default=0, type=int,
                        metavar=_('<pause length in seconds>'))
    parser.add_argument('-P', cls.PAUSE_BETWEEN_SWITCH, nargs='?', const=3, default=0, type=int,
                        metavar=_('<pause length in seconds>'))
    parser.add_argument('-m', '--monitoring-interval', default=2, type=int,
                        metavar=_('<time to wait before checking files for modifications, in seconds>'))
    parser.add_argument('-M', '--monitored-file', action='append', default=[],
                        metavar=_('<name of a file that will be checked for modifications>'),
                        help=_('This option can appear multiple times.'))
    parser.add_argument('--no-reload-when-stopped', action='store_true', default=False)
    parser.add_argument('--no-restart-after-change', action='store_true', default=False)
    parser.add_argument('--restart-after-substitution-change', action='store_true', default=False)
    parser.add_argument('--restart-on-touch', action='store_true', default=False)
    parser.add_argument('--no-restarting-message-when-not-playing', action='store_true', default=False)

    group = parser.add_mutually_exclusive_group(required=False)
    group.add_argument('-t', '--tracking-mode', choices=RuntimeOptions.TRACKING_OPTIONS)
    group.add_argument('--sequence-mode', choices=RuntimeOptions.SEQUENCE_OPTIONS)

    parser.add_argument('-F', '--feedback-mode', choices=RuntimeOptions.FEEDBACK_OPTIONS)
    parser.add_argument('-u', '--show-subst', action='store_true', default=False)
    parser.add_argument('-l', '--show-line-number', action='store_true', default=False)
    parser.add_argument('-L', '--show-total-lines', action='store_true', default=False)
    parser.add_argument('-E', '--close-at-end', action='store_true', default=False)
    parser.add_argument('-f', '--force-execution', action='store_true', default=False)
    parser.add_argument('-s', '--speed', default=DEFAULT_SPEED, type=int, metavar=_('<speed for reading, in words/minute>'))
    parser.add_argument('-,', '--stop-after-each-line', action='store_true', default=False)
    parser.add_argument('-Q', '--quit-without-prompt', action='store_true', default=False)
    parser.add_argument('-S', '--substitution-rules', '--subst', action='append',
    ...
    parser.add_argument('--transformation-rules', '--transform', action='append', metavar=_('<file with transformation rules>'),
                        default=[],
                        help=_('This option can appear multiple times. The files will be processed in the given order.'))
    parser.add_argument('--preprocess', action='append',
                        metavar=_('<a filtering command to preprocess the input> %s' % cls._SECURITY_WARNING),
                        default=[],
                        help=_('This option can appear multiple times. The filters will be applied in the given order.'))
    segmenting_options = parser.add_mutually_exclusive_group()
    segmenting_options.add_argument('--separator', default='',
                                    metavar=_('<string or regular expression to be used as a separator>'))
    segmenting_options.add_argument('--segment', default='', metavar=_('<regular expression for segmenting>'))
    parser.add_argument('--regex-flags', default='', metavar=_('<global flags for regular expressions>'))
    parser.add_argument('--no-echo', action='store_true', default=False)
    parser.add_argument('--no-showline', action='store_true', default=False)
    parser.add_argument('--no-info', action='store_true', default=False)
    parser.add_argument('--no-update-player', action='store_true', default=False)
    parser.add_argument('--no-reset-scheduled-stop-after-moving', action='store_true', default=False)
    parser.add_argument(cls.LEFT_INDENT_SWITCH, nargs='?', default=0, type=int,
                        metavar=_('<how many spaces>'))
    parser.add_argument(cls.RIGHT_INDENT_SWITCH, nargs='?', default=0, type=int,
                        metavar=_('<how many spaces>'))
  ...
  def _check_running_instance(args):
    ...
    if PSUTIL_INSTALLED:
      ...
      try:
        procs = subprocess.run(['pgrep', '-a', 'python'], text=True, capture_output=True, check=True).stdout.split('\n')
        ...
        Output.report_error(e)
        Output.say(_('The program could not check for a running instance.'), type_of_msg=Output.ERROR)
        Output.say(_('The program will assume no instance is running.'), type_of_msg=Output.INFO)
        return
    if procs:
      Output.say(_('An instance seems to be running as one of the following processes:'), type_of_msg=Output.ERROR)
      Output.say('\n'.join(pids), type_of_msg=Output.ERROR_EXTENDED)
      Output.say(_('Use the %s option to force program execution.') % '-f', type_of_msg=Output.INFO)
  ...
  def _set_encoding(cls, args):
    ...
    INPUT_ENCODING_SIG = 'utf-8-sig' if INPUT_ENCODING.lower() in ['utf-8', 'utf8'] else INPUT_ENCODING
  ...
  def _save_default_key_bindings(args):
    ...
    try:
      ...
      if output is not sys.stdout:
        Output.say(_('Default keystroke configuration was saved to file: %s\n') % filename, type_of_msg=Output.INFO)
        output.close()
      Main.terminate()
    except IOError as e:
      Output.say(_('An error has occurred while trying to save keystroke configuration to file: %s') % filename,
  ...
  def _edit_keycodes(args):
    ...
    if bindings:
      if filename == '-':
        output = sys.stdout
      elif not UserInput.confirm_action(_('Please confirm that you want to save the key bindings to file %s.') % filename):
        Main.terminate(DEFAULT_ERROR_RETURNCODE)
      else:
        try:
          current_content = ''
  ...
  def _set_language(args):
    ...
    if args.lang:
      lang = args.lang
      try:
        cp = subprocess.run(['espeak', '--voices'], text=True, capture_output=True, check=True)
        # espeak doesn't necessarily end with returncode != 0 in certain cases
        if cp.stderr:
        ...
        if lang not in valid_languages:
          ...
          valid_languages = textwrap.fill(valid_languages, Output.window_width(sys.stderr),
                                          break_on_hyphens=False)
          Main.terminate(_('The language given (%s) is not valid. Valid languages are:\n\n%s.') % (lang, valid_languages))
      except Exception as e:
        Output.report_error(e)
        Output.say(_('An error has occurred while trying to query espeak’s available languages.'), type_of_msg=Output.ERROR)
        Output.say(_('The program cannot determine if the given language is valid.'), type_of_msg=Output.ERROR)
        Output.say(_('The program will try to use the given language (%s) anyway.') % lang, type_of_msg=Output.INFO)
    else:
      lang = locale.getdefaultlocale()[0].split('_')[0]
    Output.say(_('Language used for reading: %s') % lang, type_of_msg=Output.INFO)
    LANG = lang
  ...
  def _load_language_config(cls, args):
    if args.config_file:
      ...
      fns = [sys.path[0] + '/%s.%s.cfg' % (PROGNAME, LANG), sys.path[0] + '/%s.cfg' % PROGNAME]
    for fn in fns:
      if os.path.isfile(fn):
        Output.say(_('Language configuration file: %s') % fn, type_of_msg=Output.INFO)
        cls._load_language_config_aux(fn)
        return
    Output.say(_('No language configuration file was provided.'), type_of_msg=Output.INFO)
  ...
  def _load_language_config_aux(cls, fn):
    ...
    for line in conf:
      ...
      if not m:
        Output.say(_('Wrong syntax in file: %s.') % fn, type_of_msg=Output.ERROR)
        Main.terminate(_('The wrong line is: %s') % line)
        return
      key = m.group(1)
      value = m.group(2)
      if key not in valid_keys:
        Output.say(_('Wrong option name in file: %s.') % fn, type_of_msg=Output.ERROR)
  ...
  def _read_config_file(cls, fn):
    ...
    try:
      ...
      Output.say(_('There was an error trying to read from file: %s') % fn, type_of_msg=Output.ERROR)
      Main.terminate(Main.REPORTED_ERROR_MSG % e)
  ...
  def _set_voice(cls, args):
    ...
    if args.voice:
      ...
      try:
        ...
        cp = subprocess.run(['espeak', '--voices=%s' % shlex.quote(LANG)], text=True, capture_output=True, check=True)
        if cp.stderr:
          # espeak doesn't necessarily end with returncode != 0 in certain cases
          raise Exception(cp.stderr.strip())
        tmp = cp.stdout.strip().split('\n')[1:]
        if tmp:
          ...
          tmp2 = [x for x in tmp if 'mbrola' not in x and '-mb-' not in x]
          if tmp2:
            voices.append(tmp2[0].split()[3])
        else:
          Output.say(_('No voices for language %s were found.') % LANG, type_of_msg=Output.ERROR)
      except Exception as e:
        Output.report_error(e)
        Output.say(_('An error has occurred while trying to query espeak’s available voices.'), type_of_msg=Output.ERROR)
    if voices:
      for v in voices:
        try:
          # Sanitizing 'v' is not necessary, provided the following command is given as an array. Be VERY careful to provide
          # sanitization if you were to make the following call through a shell.
          cp = subprocess.run(['espeak', '-v', v, ' '], text=True, capture_output=True, check=True)
          if cp.stderr:
            # espeak doesn't necessarily end with returncode != 0 in certain cases
            raise Exception(cp.stderr.strip())
          else:
          ...
          Output.report_error(e)
          Output.say(_('An error has occurred while testing espeak with voice %s.') % v, type_of_msg=Output.ERROR)
    if voice:
      EspeakController.voice(voice)
      Output.say(_('Voice used for reading: %s') % voice, type_of_msg=Output.INFO)
    else:
      Output.say(_('The voice for reading could not be determined, the program will try to use espeak’s default voice.'),
          type_of_msg=Output.INFO)

  # If you modify this method, be VERY careful to sanitize args.opt (shlex.split or shlex.quote where appropriate),
  # in case the call were ever to be made through a shell.
  @staticmethod # class CmdLineArgs
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _set_espeak_options(args):
    if args.opt:
      try:
        ...
        Output.say(_('Wrong options for espeak were given: %s.') % args.opt, type_of_msg=Output.ERROR)
        Main.terminate(Main.REPORTED_ERROR_MSG % e)
    ...
    if not filename:
      ...
      for binding in _bindings:
        ...
        if key in key_bindings:
          Output.say(_('The binding for key ‘%s’ has been redefined to be ‘%s’.') % (key, value), type_of_msg=Output.INFO)
  ...
  def _report_key_binding_error(e, args):
    if args.key_bindings:
      Output.say(_('Error in keystroke configuration file: %s.') % args.key_bindings, type_of_msg=Output.ERROR)
    else:
  ...
  # to avoid a 'command injection'. InputTextLoader depends on receiving a sanitized input command.
  @classmethod # class CmdLineArgs
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _get_input_source(cls, args):
    if args.do is not None:
      ...
      if LINUX:
        cls._input_command = '%s -c %s' % (os.environ['SHELL'], shlex.quote(args.do))
      elif WINDOWS:
      ...
      if LINUX:
        ...
        try:
          ...
          Output.say(_('The program will assume the input file’s mimetype is %s.') % mt, type_of_msg=Output.INFO)
      else:
        mt = 'text/plain' if cls._input_file.lower().endswith('.txt') else None
      if mt == 'text/plain':
        if LINUX:
          cls._input_command = 'cat ' + shlex.quote(cls._input_file)
        else:
          cls._input_command = 'cmd /C type ' + shlex.quote(cls._input_file)
      else:
        conversion_option = '--config-file %s ' % shlex.quote(args.text_conversion_config) if args.text_conversion_config else ''
        cls._input_command = "%s %s --lang %s %s" % (EFABCONV, conversion_option, shlex.quote(LANG), shlex.quote(cls._input_file))

    for the_filter in args.preprocess:
      # No sanitizing is possible here, it is up to the user
      cls._input_command += ' | ' + the_filter

  # If you modify this method, be VERY careful to ensure _input_command and _transformation_log_command are sanitized
  # (shlex.quote where appropriate) to avoid a 'command injection'. InputTextLoader and depends on receiving sanitized commands.
  @classmethod # class CmdLineArgs
  #@mainthreadmethod # Executed only in main thread. Uncomment to enforce check at runtime.
  def _get_transformation_rules(cls, args, flags):
    cls._transformation_rule_files = list(map(os.path.expanduser, args.transformation_rules))
    ...
    if transformation_option:
      if flags:
        transformation_option = '--regex-flags %s %s ' % (shlex.quote(flags), transformation_option)
      transformation_option += '--encoding %s --config-encoding %s ' % (shlex.quote(INPUT_ENCODING), shlex.quote(CONFIG_ENCODING))
      cls._input_command += ' | %s %s' % (EFABTRANS, transformation_option)
  ...
  SUBSTITUTION_SEPARATOR = _('A substitution expression cannot be used as a separator: %s')
  WRONG_SEGMENTING_REGEX = _('Wrong regular expression given for segmenting: %s')
  UNTERMINATED_SEGMENTING_REGEX = _('Unterminated regular expression given for segmenting: %s')
  INVALID_SEGMENTING_FLAGS = _('Invalid flags given for segmenting: %s')
  SUBSTITUTION_SEGMENTING = _('A substitution expression cannot be used for segmenting: %s')
  ...
  def _set_segmenting_mode(cls, args):
    ...
    if efabregex.is_substitution(what):
      Main.terminate((cls.SUBSTITUTION_SEPARATOR if what is args.separator else cls.SUBSTITUTION_SEGMENTING) % what)
    elif efabregex.is_unterminated(what):
      Main.terminate((cls.UNTERMINATED_SEPARATOR_REGEX if what is args.separator else cls.UNTERMINATED_SEGMENTING_REGEX) % what)
    elif efabregex.contains_invalid_flags(what):
      Main.terminate((cls.INVALID_SEPARATOR_FLAGS if what is args.separator else cls.INVALID_SEGMENTING_FLAGS) % what)
    ...
    try:
      ...
      Output.say((cls.WRONG_SEPARATOR_REGEX if what is args.separator else cls.WRONG_SEGMENTING_REGEX) % what,
                 type_of_msg=Output.ERROR)
      Main.terminate(Main.REPORTED_ERROR_MSG % e)
  ...
  def _set_substitution_rules(cls, args):
    ...
    for fn in list(map(os.path.expanduser, args.substitution_rules)):
      cls._check_file_validity(fn)
      if not fn in substitution_rule_files:
    ...
    if substitution_rule_files:
      ...
      FileMonitor.register_group(substitution_rule_files, lambda: Main.event(Main.SUBST_RULES_FILE_CHANGED))
    else:
  ...
  def _set_runtime_options(args):
    ...
    RuntimeOptions.restart_after_substitution_change(args.restart_after_substitution_change)
    RuntimeOptions.restart_on_touch(args.restart_on_touch)
    RuntimeOptions.restarting_message_when_not_playing(not args.no_restarting_message_when_not_playing)
    ...
    RuntimeOptions.reset_scheduled_stop_after_moving(not args.no_reset_scheduled_stop_after_moving)
    ...
    if args.speed < MINSPEED or args.speed > MAXSPEED:
      Main.terminate(_('The speed cannot be less than %s nor greater than %s.') % (MINSPEED, MAXSPEED))
