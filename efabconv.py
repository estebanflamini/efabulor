...
# EFABCONV: a command-line wrapper for printing different types of files to
# standard output
...
def main():
    ...
    parser.add_argument(
        "-c",
        "--config-file",
        metavar=_("<configuration file>"),
        default=get_default_conversion_rules_file(),
    )
    parser.add_argument(
        "--version", action="version", version="%(prog)s " + __version__
    )
    parser.add_argument(
        "infile", nargs="+", metavar=_("<file to read input from>")
    )
    parser.add_argument(
        "--lang", default=None, metavar=_("<language ISO code>")
    )
    ...
    if args.config_file:
        ...
        try:
            ...
            sys.exit(
                _("Cannot read configuration file: %s.") % args.config_file
            )
    else:
        conv = None
...
# If you modify this function, be VERY careful to sanitize cmd, as it will be
# executed thru a shell in run()
def process(f):
    mt = mimetype(f)
    ...
    if m:
        ...
        sys.exit(
            _(
                "Mimetype of %s is not text/plain and no configuration file "
                "for conversion was provided."
            ) % f
        )
    else:
        ...
        if ext:
            ...
            if m:
                ...
                sys.exit(
                    _("No action configured for extension %s in config file: "
                      "%s") % (ext[1:], args.config_file)
                )
        else:
...
def mimetype(f):
    if PLATFORM == "Linux":
        try:
            ...
            sys.exit(
                _("Cannot determine mimetype of file: %s. Reported error is: "
                  "%s") % (f, e)
            )
    elif PLATFORM == "Windows":
    ...
    try:
        for cmd in input_pipeline:
            result = subprocess.run(
                cmd, input=result, check=True, capture_output=True
            ).stdout
        ...
        print(
            _("Conversion failed. Reported error is: %s") % e,
            file=sys.stderr
        )
        sys.exit(1)
...
def create_input_pipeline(cmd, conversion_rule):
    try:
        ...
        print(
            _(
                "A wrong conversion command was tried as the result of a "
                "conversion rule:\n\n%s\n"
            ) % cmd,
            file=sys.stderr,
        )
        ...
        sys.exit(
            _("The rule which was set in configuration file %s is:\n\n%s\n")
